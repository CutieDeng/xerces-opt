...
ArrayDetectErrorCode runArrayDetectionPipeline (
  ...
  for (auto it = detector.m_type_field_writes->begin();
       ...
       ++it) {
    TypeFieldWriteOps* tfwo = (*it).second;
    if (!tfwo || !tfwo->write_ops) continue;

    // 遍历该字段的所有写入操作
    for (unsigned i = 0; i < tfwo->write_ops->length(); i++) {
      FieldWriteCapture* write_capture = (*tfwo->write_ops)[i];
      if (!write_capture) continue;

      // 执行源操作数使用分析
      SourceUseAnalysisResult* use_result =
        analyzeFromWriteCapture(write_capture, escape_rules);
      ...
      if (use_result) {
        ...
        if (ctx.debug_file && use_result->has_escape) {
          const char* type_name = IDENTIFIER_POINTER(DECL_NAME(TYPE_NAME((*it).first.type)));
          const char* field_name = IDENTIFIER_POINTER(DECL_NAME((*it).first.field_decl));
