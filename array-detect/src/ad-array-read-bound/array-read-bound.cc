    ...
    while (unwrap_depth < 5) {
      ...
      if (TREE_CODE (actual_index) == SSA_NAME) {
        ...
        if (rhs_code == MULT_EXPR) {
          ...
          AD_DEBUG_PRINT ("[read-bound] Step 3: unwrap SSA->MULT_EXPR depth=%d, next code=%d",
          ...
          // 类型转换：取内部操作数
          actual_index = gimple_assign_rhs1 (def);
          AD_DEBUG_PRINT ("[read-bound] Step 3: unwrap SSA->CONVERT depth=%d, next code=%d",
                          unwrap_depth, TREE_CODE (actual_index));
          unwrap_depth++;
          continue;
        } else {
      ...
      // 处理直接的 MULT_EXPR
        ...
        AD_DEBUG_PRINT ("[read-bound] Step 3: unwrap direct MULT_EXPR depth=%d, next code=%d",
                        unwrap_depth, TREE_CODE (actual_index));
        unwrap_depth++;
        continue;
      }
      // 处理直接的类型转换
        ...
        actual_index = TREE_OPERAND (actual_index, 0);
        AD_DEBUG_PRINT ("[read-bound] Step 3: unwrap direct CONVERT depth=%d, next code=%d",
                        unwrap_depth, TREE_CODE (actual_index));
        unwrap_depth++;
        continue;
      }
