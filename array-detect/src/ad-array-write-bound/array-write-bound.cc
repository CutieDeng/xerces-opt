...
ArrayDetectErrorCode findWriteDominatingConditions (
  ...
  *result = NULL;
  ...
  AD_DEBUG_PRINT ("[write-bound] findWriteDominatingConditions: access at %s, bb=%d",
  ...
  AD_DEBUG_PRINT ("[write-bound] findWriteDominatingConditions: %u predecessors, %u conditions found",
...
ArrayDetectErrorCode analyzeWriteAccessToEvidences (
  ...
  *results = NULL;
  ...
  // 例如: fElemList[fCurCount] = x; 其中 index 就是 fCurCount
  ...
  if (index_to_check && TREE_CODE (index_to_check) == INTEGER_CST) {
    ...
    while (base && TREE_CODE (base) == SSA_NAME && depth < 10) {
      ...
      AD_DEBUG_PRINT ("[write-bound]   depth=%d: rhs_code=%d (POINTER_PLUS=%d)",
                      depth, (int)rhs_code, (int)POINTER_PLUS_EXPR);

      // 找到 POINTER_PLUS_EXPR
      if (rhs_code == POINTER_PLUS_EXPR) {
        index_to_check = gimple_assign_rhs2 (def);  // 获取偏移部分（第二个操作数）
        AD_DEBUG_PRINT ("[write-bound] found POINTER_PLUS_EXPR at depth=%d, index code=%d",
  ...
  if (index_to_check && access->containing_type) {
    ...
    for (tree field = TYPE_FIELDS (access->containing_type); field; field = DECL_CHAIN (field)) {
      ...
      if (exprReferencesField (AD_ARGS, index_to_check, field)) {
        ...
        evidence->write_access = access;
        evidence->bound_condition = NULL;  // 无边界条件，仅通过 index 引用
        evidence->confidence = WRITE_CONF_PROBABLE;  // 置信度稍低
