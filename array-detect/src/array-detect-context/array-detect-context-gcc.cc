...
static char const * demangle_name (char const * mangled, char * buffer, size_t buffer_size) {
...
// 使用新地址解析器的增强版本
ArrayDetectErrorCode resolveFrameAddressToSource (AD_FUNC_ARGS, uint64_t frame_addr, char const * &result, bool &out_is_valid) AD_FUNCTION_BEGIN {
  AD_ARGS_WARN_DENY;
  ...
  if (dladdr ((void*)frame_addr, &info) && info.dli_sname) {
    ...
    char const * demangled = demangle_name (info.dli_sname, demangle_buffer, sizeof (demangle_buffer));
