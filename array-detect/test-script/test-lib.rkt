...
(define (parse-ut-compile-task task cxx other-args root-path)
  ...
  `(,cxx "-c" ,@(force other-args) ,@(force args) ,(build-path (force root-path) (force source)) "-o" ,(build-path (force root-path) (force target)))
)

(define (parse-total-compile-task task cxx other-args root-path)
  (match-define (total-compile-task sources target args) task)
  `(,cxx ,@(force other-args) ,@(force args) ,@(map (lambda (s) (build-path (force root-path) s)) (force sources)) "-o" ,(build-path (force root-path) (force target)))
)

(define (run-test config cxx other-args root-path)
  (match-define (test-config name pre-fn post-fn ut-compile-tasks total-compile-task _) config)
