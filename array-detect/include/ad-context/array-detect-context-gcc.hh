...
ArrayDetectErrorCode resolveFrameAddressToSource (AD_FUNC_ARGS, uint64_t frame_addr, char const * &result, bool &out_is_valid);

// 当前栈帧信息结构
struct CurrentFrameInfo {
  uint64_t frame_address;        // 栈帧地址
  bool is_valid;                  // 是否有效
  char const * source_location;    // 源码位置信息（使用上下文缓冲区）
  char const * demangled_name;    // 解析后的函数名（使用上下文缓冲区）
  char const * source_file;        // 源码文件名（使用上下文缓冲区）
