...
struct AssignmentDetail {
  char const * source;              // 赋值来源（ggc_strdup 分配）
  bool is_call;                    // 是否是函数调用
  int tree_code;                   // 右值表达式的树代码
  char const * location_info;       // 位置信息（ggc_strdup 分配）
  char const * rhs_description;     // 右值表达式描述（ggc_strdup 分配）
};

// 函数级别的赋值信息
// 垃圾回收：所有字段由 GCC 自动管理
struct FunctionAssignment {
  char const * function_name;      // 函数名称（ggc_strdup 分配）
  tree function_decl;              // 函数声明（GCC 内部管理）
  char const * function_id;        // 函数唯一标识字符串（ggc_strdup 分配）
  int assignment_count;            // 该函数中该字段的赋值次数
  vec<char const *>* sources;       // 赋值来源向量（ggc_alloc<vec<...>>() 分配）
  vec<AssignmentDetail*>* assignment_details; // 详细赋值信息向量（ggc_alloc<vec<...>>() 分配）
};

struct FieldInfo {
  char const * field_name;         // 字段名称（ggc_strdup 分配）
  char const * containing_type;    // 包含该字段的类型名称（ggc_strdup 分配）
  ...
  vec<char const *>* sources;       // 赋值来源向量（ggc_alloc<vec<...>>() 分配）
  vec<char const *>* conflicting_assigns; // 冲突赋值向量（ggc_alloc<vec<...>>() 分配）
